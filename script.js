document.addEventListener('DOMContentLoaded', () => {
    ...
    // Referências para busca e filtros
    ...
    // Elementos e variáveis para paginação
    const paginationControls = document.getElementById('pagination-controls');
    let currentPage = 1;
    const productsPerPage = 9;
    const BASE_API_URL = "https://catalogo-backend-e14g.onrender.com/api";
        ...
        try {
            ...
            if (existingItem) {
                if (existingItem.quantity < productToAdd.stock) {
                    ...
                    alert(`Quantidade de "${productToAdd.name}" atualizada para ${existingItem.quantity} no carrinho!`);
                } else {
    ...
    function updateQuantity(productId, change) {
        ...
        if (change > 0 && newQuantity > item.stock) {
            alert(`Não há estoque suficiente para adicionar mais de "${item.name}". Estoque disponível: ${item.stock}.`);
            return;
        }
        ...
        try {
            ...
            if (!Array.isArray(products) || products.length === 0) {
                if (currentSearchTerm || currentCategoryFilter !== 'all' || currentMinPrice || currentMaxPrice || currentMinStock) {
                    ...
                    productsContainer.innerHTML = '<p class="info-message">Nenhum produto cadastrado ainda. Adicione um!</p>';
                }
            } else {
            ...
            productsContainer.innerHTML = '<p class="error-message">Erro ao carregar produtos. Verifique se o servidor está rodando.</p>';
    ...
    addProductForm.addEventListener('submit', async (event) => {
        ...
        const url = id ? `${BASE_API_URL}/products/${id}` : `${BASE_API_URL}/products`;
    ...
    window.editProduct = async (id) => {
        try {
            ...
            if (!response.ok) throw new Error(`Erro ao buscar produto para edição: status ${response.status}`);
    ...
    window.deleteProduct = async (id) => {
        ...
        try {
            const response = await fetch(`${BASE_API_URL}/products/${id}`, { method: 'DELETE' });
            if (response.ok) {
    ...
    // Função para aplicar todos os filtros e buscar
    ...
    // Event listeners dos botões Buscar e Limpar Busca
    ...
    // Delegação de evento para botões "Adicionar ao Carrinho"
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btnShowAdminPanel.addEventListener("click", () => {
    ...
    toggleAdminButtons();  // Mostrar botões Editar/Excluir
  });
  ...
  btnHideAdminPanel.addEventListener("click", () => {
    ...
    toggleAdminButtons();  // Ocultar botões Editar/Excluir
  });
